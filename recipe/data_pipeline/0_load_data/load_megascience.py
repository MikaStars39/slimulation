...
def load_klearreasoner(
    ...
    for idx, row in enumerate(tqdm(dataset, desc=f"Loading {dataset_name}")):
        ...
        row.pop("reference_answer")
        full_answer = row["answer"]
        row.pop("answer")
        raw_source = row["source"]
        row.pop("source")

        if row["subject"] != "" and row["subject"] != "math":
            for sample_idx in range(k):
                # Create a unique ID for each attempt
                # Format: {dataset}_{original_index}_{attempt_index}
                unique_id = f"MegaScience-stem_{idx}_{sample_idx}"
                
                record = {
                    "id": unique_id,
                    "question_id": f"MegaScience-stem_{idx}",
                    "source": "MegaScience-stem",
                    "prompt": question,
...
if __name__ == "__main__":
    output_dir = "/mnt/llm-train/users/explore-train/qingyu/data/stage_1/MegaScience"

    # first make dictionary
    os.makedirs(output_dir, exist_ok=True)
    
    load_klearreasoner(
        dataset_name="/mnt/llm-train/users/explore-train/qingyu/.cache/MegaScience",
        k=1,
