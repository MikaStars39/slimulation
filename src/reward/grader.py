...
def _normalize(expr: str) -> str:
    ...
    # Normalize common scientific notation forms early so sympy can compare them reliably.
    # Examples:
    # - 1.325e-27
    # - 1.325 \\times 10^{-27}
    # - 1.325 \\cdot 10^{-27}
    #
    # We rewrite "\\times 10^{k}" / "\\cdot 10^{k}" into "*10^k" BEFORE latex-to-text,
    # because latex2text may emit unicode superscripts (e.g. 10⁻²⁷) that sympy can't parse.
    expr = expr.replace("\\cdot", "\\times")
    expr = re.sub(
...
def main():
    # Example usage
    ground_truth = "1.325e-27"
    given_answer = "\\boxed{1.325 \\times 10^{-27}}"
    score, valid = grade_answer(given_answer, ground_truth)
    print(f"score={score}, valid={valid}")
