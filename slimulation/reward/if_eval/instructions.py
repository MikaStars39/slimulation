...
class KeywordFrequencyChecker(Instruction):
  ...
  def build_description(self, *, keyword = None,
    ...
    if self._frequency is None or self._frequency < 0:
      self._frequency = random.randint(1, 3)

    if relation is None:
      self._comparison_relation = random.choice(("less than", "at least"))
    elif relation not in _COMPARISON_RELATION:
      raise ValueError("The supported relation for comparison must be in "
                       f"{_COMPARISON_RELATION}, but {relation} is given.")
    else:
...
class RepeatPromptThenAnswer(Instruction):
  ...
  #   if self._description_pattern in value.lower():
  #     return False

  #   norm_value = value.strip()
  #   norm_prompt = self._prompt_to_repeat.strip()

  #   if not norm_value.lower().startswith(norm_prompt.lower()):
  #     return False

  #   answer_part = norm_value[len(norm_prompt):]
  ...
  def check_following(self, value):
    """
    Checks if the response repeats the prompt exactly once, then provides an answer.
    This version performs a strict, case-sensitive check.
    """
    # The response should not contain the instruction pattern itself.
    if self._description_pattern in value:
        # Note: This check remains case-insensitive to robustly catch the instruction pattern
        if self._description_pattern.lower() in value.lower():
            return False
    ...
    # Condition 1: The response must start with the prompt (case-sensitive).
    if not norm_value.startswith(norm_prompt):
      return False

    # Isolate the part of the response that comes AFTER the first prompt repetition.
    answer_part = norm_value[len(norm_prompt):].strip()

    # Condition 2: There must be a non-empty answer.
    if not answer_part:
      return False

    # Condition 3: The answer part must NOT start with the prompt again (case-sensitive).
    if answer_part.startswith(norm_prompt):
      return False

    # If all checks pass, the response follows the instruction correctly.
    return True
